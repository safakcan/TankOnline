...
struct MessagePoint: MessageProtocol{
    var message: Message
    var amount: Int
}
...
class MultiplayerNetworking: NSObject, GameKitHelperDelegate {
    ...
    func match(match: GKMatch, data: Data, playerID: String) {
        ...
        if message?.messageType == .kMessageTypeRandomNumber {
            ...
            delegate?.movePlayer(index: indexForPlayer(playerID: playerID), position: messageMove!.position)
        }
            ...
            print("Score message received")
            let messageScore: MessagePoint? = data.withUnsafeBytes { $0.pointee }
            delegate?.changePoint(index: indexForPlayer(playerID: playerID), amount: messageScore!.amount)
        }
    ...
    func sendPoint(amount: Int) {
        var message: MessagePoint = MessagePoint(message: Message(messageType: .kMessageTypePoint), amount: amount)
        let data = Data(bytes: &message, count: MemoryLayout<MessagePoint>.size)
        send(data: data)
    }
    ...
    func isLocalPlayerPlayer1() -> Bool {
        let dictionary = orderOfPlayers[0]
